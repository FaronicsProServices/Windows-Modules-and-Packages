...
# 3. Locate and install Fusion silently using /Type Silent, then verify installation
try {
    ...
    Write-Host "Installing $($exeFile.Name) silently using /Type Silent..."
    ...
    $installed = $false
    $maxAttempts = 24  # Wait up to 2 minutes (24 * 5 seconds) for files to populate
    $attempt = 0
