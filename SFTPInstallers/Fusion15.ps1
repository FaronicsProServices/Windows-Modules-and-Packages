...
# 3. Locate and install Fusion using official /type silent switch and actively monitor files
try {
    ...
    Write-Host "Launching $($exeFile.Name) using official /type silent switch..."
    # Launch wrapper (wrapper exits immediately after handing off to background setup)
    Start-Process -FilePath $exeFile.FullName -ArgumentList "/type silent"

    Write-Host "Waiting for Freedom Scientific background installer to write files..."
    Start-Sleep -Seconds 20 # Initial head start for extraction

    $fsInstallPath = "${env:ProgramFiles}\Freedom Scientific"
    $installed = $false
    $maxAttempts = 24  # Check every 5 seconds for up to 2 minutes
    $attempt = 0
    ...
    if (-not $installed) {
        throw "Installation verification failed: Freedom Scientific application files were not found on disk after waiting."
    }
