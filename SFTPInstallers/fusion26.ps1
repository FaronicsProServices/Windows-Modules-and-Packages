...
# GitHub location containing WinSCP.exe and WinSCPnet.dll
$GitHubBaseUrl = `
...
try {
    ...
    # Use the WinSCP executable downloaded above.
    $SessionOptions.SshHostKeyPolicy =
        $null

    # Accept any SSH host key.
    # Replace with a fixed fingerprint for hardened production use.
...
try {
    ...
    if (-not $LicenseSource) {
        ...
        Write-Log "License file not found."

        # Show any .lic files for troubleshooting.
        $LicFiles =
            Get-ChildItem `
                -Path $ExtractDir `
                -Filter "*.lic" `
        ...
        foreach ($LicFile in $LicFiles) {
            ...
            Write-Log "Found .lic file: $($LicFile.FullName)"
        }
...
try {
    ...
    # Same approach as the manager's working script.
    Copy-Item `
