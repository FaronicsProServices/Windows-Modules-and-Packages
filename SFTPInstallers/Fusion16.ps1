...
# 3. Locate and install Fusion using your exact specified switches
try {
    ...
    Write-Host "Installing $($exeFile.Name) using specified switches..."
    $process = Start-Process -FilePath $exeFile.FullName -ArgumentList "/Type Silent /Log `"$nativeLogPath`"" -PassThru -Wait
    ...
    if (-not $installed) {
        if (Test-Path $nativeLogPath) {
            ...
            Write-Host "Log file at $nativeLogPath was not generated by the installer."
        }
