...
# ============================================================
# PREPARE FUSION INSTALL LOG
# ============================================================

try {

    Write-Log "Preparing Fusion installer log..."

    $FusionLogDirectory =
        "$env:WINDIR\Temp"

    if (-not (Test-Path -LiteralPath $FusionLogDirectory)) {

        New-Item `
            -Path $FusionLogDirectory `
            -ItemType Directory `
            -Force |
            Out-Null
    }

    if (Test-Path -LiteralPath $FusionLogPath) {
    ...
    Write-Log $FusionLogPath

}
catch {

    Stop-WithError `
        "Unable to prepare Fusion installer log: $($_.Exception.Message)"
}
...
try {
    ...
    # IMPORTANT:
    # %WINDIR% is resolved by PowerShell before the installer
    # is launched. The effective switch remains:
    #
    # /Type Silent
    # /DisableExternalServices
    # /Log "C:\Windows\Temp\Fusion2026-Install.log"

    $InstallerArguments =
        "/Type Silent /DisableExternalServices /Log `"$FusionLogPath`""
    ...
    if ($Process.ExitCode -ne 0) {
        ...
        Write-Log `
            "Fusion installer log: $FusionLogPath"
...
try {
    ...
    # Same methodology as the manager's working script.
    Copy-Item `
    ...
    if (-not (Test-Path -LiteralPath $InstalledLicense)) {
        ...
        throw "License file was not found after copy."
    }
...
Write-Log "Fusion install log  : $FusionLogPath"
