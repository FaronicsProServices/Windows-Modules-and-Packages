...
try {
    # 1. Parse and extract the root share path (\\server\share) for net use
    if ($UncPath -match '^(\\\\[^\\]+\\[^\\]+)') {
    ...
    # 2. Establish network share connection if credentials are provided
    if (-not [string]::IsNullOrWhiteSpace($Username) -and -not [string]::IsNullOrWhiteSpace($Password)) {
        ...
        $netUseArgs = @('use', $TargetShare, $Password, "/user:$Username")
    ...
    # 3. Locate the ZIP file on the share
    ...
    # 4. Create clean local staging directory
    ...
    # 5. Set location to the extracted directory to match .\ relative paths
    Set-Location -Path $ExtractDir

    # 6. Install Freedom Scientific Fusion
    ...
    # 8. Cleanup staging artifacts and network connection
