...
$nativeLogPath = "C:\Windows\Temp\Fusion2026-Install.log"
...
# 3. Locate and install Fusion using explicit array arguments to prevent parameter parsing drops
try {
    ...
    if (Test-Path $nativeLogPath) {
        Remove-Item $nativeLogPath -Force -ErrorAction SilentlyContinue
    }

    Write-Host "Installing $($exeFile.Name) silently..."
    # Passing arguments as an explicit array prevents string-mangling under the SYSTEM account
    $process = Start-Process -FilePath $exeFile.FullName -ArgumentList ("/quiet", "/norestart", "/Log", "$nativeLogPath") -PassThru -Wait
    
    Write-Host "Installer process exited with code: $($process.ExitCode)"

    Write-Host "Waiting for Freedom Scientific background files to populate..."
    Start-Sleep -Seconds 15

    $fsInstallPath = "${env:ProgramFiles}\Freedom Scientific"
    $installed = $false
    ...
    while (-not $installed -and $attempt -lt $maxAttempts) {
        ...
        if (Test-Path $fsInstallPath) {
            $subItems = Get-ChildItem -Path $fsInstallPath -ErrorAction SilentlyContinue
    ...
    if (-not $installed) {
        if (Test-Path $nativeLogPath) {
            Write-Host "--- FUSION INSTALL LOG CONTENT ---"
            Get-Content $nativeLogPath -Tail 40 | Write-Host
            Write-Host "----------------------------------"
        }
        throw "Installation verification failed: Freedom Scientific application files were not found on disk."
    }
    
    Write-Host "Application installation verified successfully."
}
catch {
    Write-Error "Error during installation or verification: $_"
    exit 1
}

# 4. Locate and copy the license file only after confirmed installation
try {
    ...
    if (-not $licFile) {
        ...
        throw "License file '$LicenseFileName' not found inside extracted package. Available files were: $($allExtracted -join ', ')"
    }
