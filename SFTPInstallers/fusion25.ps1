...
# 3. Locate and install Fusion using your exact baseline switches
try {
    ...
    # Execute using your exact requested switches and explicit working directory context
    Start-Process -FilePath $exeFile.FullName -ArgumentList "/type Silent /DisableExternalServices" -WorkingDirectory $exeFile.DirectoryName -Wait
    ...
    $installed = $false
    $maxAttempts = 24  # Wait up to 2 minutes for files to populate
    $attempt = 0
