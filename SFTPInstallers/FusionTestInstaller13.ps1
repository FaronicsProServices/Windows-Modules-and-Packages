...
# 3. Locate and install Fusion using enterprise Inno Setup silent switches
try {
    ...
    Write-Host "Installing $($exeFile.Name) using enterprise silent switches..."
    # Using full Inno Setup command-line parameters for background/SYSTEM deployment
    Start-Process -FilePath $exeFile.FullName -ArgumentList "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP-" -Wait
    
    Write-Host "Verifying if the application files have been installed..."
    
    $fsInstallPath = "${env:ProgramFiles}\Freedom Scientific"
    $installed = $false
    $maxAttempts = 30  # Wait up to 2.5 minutes for files to populate
    $attempt = 0
    ...
    if (-not $installed) {
        throw "Installation verification failed: The Freedom Scientific directory was not populated."
    }
...
# 4. Proceed to locate and copy the license file only after confirmed installation
try {
