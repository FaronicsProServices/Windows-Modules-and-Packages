...
# This block MUST come before any calls to Write-Log
if (-not (Test-Path $LogPath)) {
    try {
        New-Item -ItemType Directory -Path $LogPath -ErrorAction Stop | Out-Null
    }
    catch {
        # If directory creation fails, we can't write to the log file, so output to console only
        Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [ERROR] Failed to create log directory $LogPath. Error: $_"
        exit 1 # Exiting is safer if logging is critical
    }
}
...
$7ZipInstalled = $false
$Current7ZipPath = "" # Initialize $Current7ZipPath
...
if (-not $7ZipInstalled) {
    ...
    try {
        ...
        elseif (Test-Path $7ZipExePath_x86) { $Current7ZipPath = $7ZipExePath_x86 }
             ...
             Write-Log "7-Zip was installed, but executable not found at expected paths. This might cause issues. Attempting to find via Get-Command." -Level "WARN"
             ...
             if (-not $Current7ZipPath) {
                 Write-Log "Failed to locate 7z.exe after installation. Extraction will likely fail. Exiting." -Level "ERROR"
                 exit 1
             }
             Write-Log "Found 7z.exe at: $Current7ZipPath" -Level "INFO"
        }
    }
    catch {
...
# --- Debugging: Verify Downloaded ZIP file ---
Write-Log "Verifying downloaded ZIP file: $ReadWriteDownloadPath" -Level "INFO"
if (Test-Path $ReadWriteDownloadPath) {
    $zipFileSize = (Get-Item $ReadWriteDownloadPath).Length
    Write-Log "Downloaded ZIP file size: $([math]::Round($zipFileSize / 1GB, 2)) GB" -Level "INFO"
    # A 1GB file is 1,073,741,824 bytes. Allowing a small tolerance for network overhead/compression differences.
    if ($zipFileSize -lt 900MB) { # If it's significantly less than 1GB, it might be incomplete
        Write-Log "WARNING: Downloaded ZIP file seems unusually small ($([math]::Round($zipFileSize / 1MB, 2)) MB). It might be incomplete or corrupted." -Level "WARN"
    }
} else {
    Write-Log "ERROR: Downloaded ZIP file does not exist at $ReadWriteDownloadPath after download attempt." -Level "ERROR"
    exit 1
}

# STEP 4: Extract the Read&Write ZIP file using 7-Zip
Write-Log "Attempting to extract ZIP using 7-Zip from: $Current7ZipPath" -Level "INFO"
Write-Log "7-Zip arguments: x `"$ReadWriteDownloadPath`" -o`"$ReadWriteExtractPath`" -y" -Level "INFO"
Write-Log "This might take a while due to the large file size and numerous files. Please wait..." -Level "INFO"
try {
    ...
    if ($exitCode -eq 0) {
        Write-Log "7-Zip extraction complete (Exit Code: $exitCode)." -Level "INFO"
    }
    else {
        Write-Log "7-Zip extraction failed with exit code: $exitCode. This indicates a problem with the ZIP or 7-Zip execution." -Level "ERROR"
        # If possible, get stderr/stdout from the process for more details (requires capturing streams)
        # For simplicity, relying on exit code and manual checks for this script
        exit 1
    }
}
catch {
    Write-Log "Failed to execute 7-Zip for extraction. Error: $_" -Level "ERROR"
    exit 1
}

# --- Debugging: Verify Extracted MSI file ---
Write-Log "Verifying extracted MSI file: $ReadWriteMsiPath" -Level "INFO"
if (Test-Path $ReadWriteMsiPath) {
    Write-Log "Extracted MSI file found." -Level "INFO"
} else {
    Write-Log "ERROR: Extracted MSI file '$ReadWriteMsiFileName' NOT FOUND at $ReadWriteMsiPath after extraction attempt." -Level "ERROR"
    # List contents of extract directory to help debug
    Write-Log "Listing contents of extraction directory: $ReadWriteExtractPath" -Level "ERROR"
    Get-ChildItem -Path $ReadWriteExtractPath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
...
Write-Log "Proceeding with MSI installation..." -Level "INFO"
...
if (Test-Path $ReadWriteMsiPath) {
    ...
    # This block should now ideally not be hit if the prior "Verify Extracted MSI file" check is working.
    Write-Log "ERROR: Read&Write MSI file '$ReadWriteMsiFileName' not found at $ReadWriteMsiPath. This indicates a problem with the extraction or file integrity." -Level "ERROR"
    exit 1
}
