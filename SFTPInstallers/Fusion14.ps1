...
# 3. Locate and install Fusion using official /type silent switch and monitor background setup
try {
    ...
    Write-Host "Installing $($exeFile.Name) using official /type silent switch..."
    # Launch the wrapper with the official switch
    Start-Process -FilePath $exeFile.FullName -ArgumentList "/type silent" -Wait
    
    Write-Host "Waiting for background installer processes to finish..."
    # Freedom Scientific wrappers spawn background setup engines; wait for them to settle
    Start-Sleep -Seconds 15

    $maxWait = 36 # Up to 3 minutes
    $waited = 0
    while ((Get-Process -Name "*setup*", "*fusion*", "*fsinstall*" -ErrorAction SilentlyContinue) -and ($waited -lt $maxWait)) {
        Start-Sleep -Seconds 5
        $waited++
    }

    # Verify installation directory is populated
...
# 4. Locate and copy the license file only after verified installation
try {
